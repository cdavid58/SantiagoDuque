...
def home(request):
	...
	if request.is_ajax():
		...
		if request.GET.get("a") is not None or request.GET.get("b") is not None:
			sms = ""
			try:
				sus = Suscripcion.objects.get(Q(email=request.GET.get("a")) | Q(telefono=request.GET.get("a")))
	...
	return render(request,'home/index.html',{'categoria':cat,'products':products,'subcategoria':subcategoria,'info':info})
...
def detailscart(request):
	...
	return render(request,'home/cart.html',{'cart':cart,'categoria':cat,'subcategoria':subcategoria,'products':products})
